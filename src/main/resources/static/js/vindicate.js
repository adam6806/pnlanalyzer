    ...
    this.options = $.extend(true, {
        ...
        formats: {
            ...
            password: {
                ...
                message: "Your password must be at least 8 characters long and contain a lower case letter, uppercase letter, and a number."
            }
        }
    }, options);
...
function vindicateField($element, formId, options) {
    ...
    if (this.element.is(":text") || this.element.is("textarea") || this.element.is("[type=email]") || this.element.is("[type=date]") || this.element.is("[type=password]")) {
        this.fieldType = "text"
    }
    ...
    this.validateFormat = function (options) {
        strict_validation = ["alpha", "alphanumeric", "creditcard", "date", "decimal", "email", "numeric", "phone", "time", "url", "password"];
